...
async function readPacket(): Promise<Packet> {
  ...
  const type = (await readBitsAsNumber(3)) as OpType | PacketType.Literal;
  switch (type) {
...
function valueOfPacket(p: Packet): number {
  if (p.type === PacketType.Literal) {
    return p.value;
  }
  switch (p.operationType) {
    case OpType.Sum:
      return p.contents.reduce((sum, cur) => sum + valueOfPacket(cur), 0);
    case OpType.Prod:
      return p.contents.reduce((prod, cur) => prod * valueOfPacket(cur), 1);
    case OpType.Min:
      return Math.min(...p.contents.map(valueOfPacket));
    case OpType.Max:
      return Math.max(...p.contents.map(valueOfPacket));
    case OpType.Gt:
      return valueOfPacket(p.contents[0]) > valueOfPacket(p.contents[1])
        ? 1
        : 0;
    case OpType.Lt:
      return valueOfPacket(p.contents[0]) < valueOfPacket(p.contents[1])
...
console.log(valueOfPacket(transmission));
